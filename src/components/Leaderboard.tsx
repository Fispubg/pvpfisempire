...
import { Zap, Trophy, Globe, Sword, Shield, Cpu, Wind, ChevronDown, ChevronUp } from "lucide-react";
...
const mockPlayers = [
  { rank: 1, name: "PrimeFis", mcName: "PrimeFis", skills: { weapons: [20, 15, 10, 15, 20, 10, 15], tyrant: [15, 10, 10, 10, 10, 10, 20, 15, 10, 15], survivor: [10, 15, 40, 10, 15], mechanics: [30, 20, 15, 20, 15] } },
  { rank: 2, name: "Dream", mcName: "Dream", skills: { weapons: [12, 10, 6, 8, 12, 5, 8], tyrant: [8, 6, 6, 6, 7, 6, 12, 9, 7, 9], survivor: [6, 9, 28, 6, 9], mechanics: [18, 12, 9, 12, 9] } },
];
...
const Leaderboard = () => {
  ...
  return (
    <section className="py-20 px-4 bg-[#0a0a0a] min-h-screen font-display">
        ...
        <div className="bg-[#111111] rounded-b-2xl border border-white/5 divide-y divide-white/5 overflow-hidden shadow-2xl">
          {mockPlayers.map((player) => (
            <div key={player.rank} className="group">
              <div
                onClick={() => setExpandedPlayer(expandedPlayer === player.rank ? null : player.rank)}
                className={`grid grid-cols-12 gap-2 px-8 py-6 items-center cursor-pointer transition-all duration-300 hover:bg-white/[0.04] ${expandedPlayer === player.rank ? 'bg-primary/5 border-l-4 border-primary' : 'border-l-4 border-transparent'}`}
              >
                <div className="col-span-1 text-center font-black italic text-lg text-gray-500">#{player.rank}</div>
                
                {/* Player Face + Name */}
                <div className="col-span-4 flex items-center gap-4">
                  <img 
                    src={`https://mc-heads.net/avatar/${player.mcName}/100`} 
                ...
                <div className="col-span-2 text-center font-bold text-gray-400">N/A</div>
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-black/50 border-t border-white/5">
                    <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
                      ...
                      <div className="flex flex-col items-center justify-center p-6 bg-white/[0.03] rounded-3xl border border-white/5">
                         <img 
                           src={`https://mc-heads.net/body/${player.mcName}/right`} 
                      ...
                      <div className="space-y-4 max-h-[450px] overflow-y-auto pr-4 custom-scrollbar text-left">
                         {Object.entries(SKILL_CATEGORIES).map(([key, cat]) => {
                           ...
                           const catTotal = scores.reduce((a: number, b: number) => a + b, 0);
                           return (
                                 ...
                                 {cat.skills.map((s, i) => (
                                   <div key={s.name}>
                                     <div className="flex justify-between text-[9px] uppercase font-bold text-muted-foreground mb-1">
                                        <span>{s.name}</span>
                                        <span>{scores[i]}/{s.max}</span>
                                     </div>
                                     <SkillBar value={scores[i]} max={s.max} />
                                   </div>
                                 ))}
                               </div>
                             </div>
